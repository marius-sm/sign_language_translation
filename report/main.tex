...
\end{table}
Table \ref{tab:2d_keypoints} (resp. Table \ref{tab:3d_keypoints}) shows the performances of the five best experiments obtained when concatenating the same 2D CNN embeddings with the 2D (resp. 3D) body keypioints extracted using DOPE. 
\begin{table}[h]
	\centering
	\def\arraystretch{0.9}
	\begin{tabular}{*5c}
